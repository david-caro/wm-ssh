#!/bin/bash
set -eo pipefail
# Script to run CI checks on your local wmcs-backups code.
# It uses the same docker image we use to run such tests in
# CI.

usage() {
    cat <<USG
$0 - run tests on your wmcs-backups working directory.
...
REPO_DIR="${SCRIPT_DIR}/.."
find "${REPO_DIR}/wmcs_backups" -type d -name __pycache__ -exec chmod -R 0777 {} +
DOCKER_TMP_DIR="${REPO_DIR}/.docker_tmp"
mkdir -p "${DOCKER_TMP_DIR}"/{.tox,doc,.eggs,wmcs_backups.egg-info}
...
mkdir -p "${REPO_DIR}"/{.tox,.eggs,wmcs_backups.egg-info}
mkdir -p "${REPO_DIR}/doc/build"
# We could probably make this clener?
find "${DOCKER_TMP_DIR}" \! -user nobody -exec chmod 0777 {} +
find "${REPO_DIR}"/.tox \! -user nobody -exec chmod 0777 {} +
# The following may be required depending on the users umask
find "${REPO_DIR}" \! -perm -o=r -exec chmod o+r {} +
find "${REPO_DIR}" -type d \! -perm -o=x -exec chmod o+x {} +

IMG_VERSION=${IMG_VERSION:-latest}
IMG_NAME=${IMG_NAME:-docker-registry.wikimedia.org/releng/tox-poetry:$IMG_VERSION}
CONT_NAME=wmcs-backups-tests-${IMG_VERSION}
...
docker run \
    ...
    --volume /"${DOCKER_TMP_DIR}/wmcs_backups.egg-info"://src/wmcs_backups.egg-info:rw \
    "$IMG_NAME" \
    "$@"
